import { PolygonGeoJsonOptions, PolygonSvgOptions, PolygonSvgOptionsSources } from './payload/polygonRequestPayload'
...
describe('TargomoClient polygon service', () => {
  ...
  test('svg request alt', async () => {
    const sources = [{ lng: 13.3786431, lat: 52.4668237, id: 1 }]

    const options4: PolygonSvgOptionsSources = {
      sources,
      serializer: 'json',
      travelType: 'walk',
      travelEdgeWeights: [300, 600],
    ...
    const result4 = <PolygonArray>await testClient.polygons.fetch(options4)
    result4.forEach((result) => {
      expect(result).toHaveProperty('area')
      expect(result.area).toBeGreaterThan(0)
      expect(result).toHaveProperty('polygons')
  ...
  test('svg request geometryies', async () => {
