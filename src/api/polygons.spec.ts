...
import { PolygonArray, BoundedPolygonSvgResult } from './polygons';
...
describe('TargomoClient polygon service', () => {
  ...
  test('enhance REST request polygons', async () => {
    ...
    const result5 = PolygonArray.create();
    (restMock.data as PolygonSvgResult[]).forEach((polygons: any) => result5.push(new BoundedPolygonSvgResult(polygons)))
