 ...
 * @param {number} optimizationTime Specifies the desired maximum run time in seconds this request can use for the optimization.
 ...
 * @param {string} optimizationAlgorithm Specifies which optimization algorithm is to be used. The following options are available:
 ...
 * @param {number} unimprovedWaitingTime An optional setting that can be used to define whether the CONSTRAINT_SATISFACTION optimization
 ...
 * optimizationTime needs to be used up. The value is in seconds. If no value is specified, the optimization is not terminated prematurely.
 * @param {string} costMatrixSource An optional setting that can be used to define how the cost of the connection matrix is calculated.
 * Options are:
 * - AIR_DISTANCE – the connection costs are determined by the air line distance between the two respective addresses
 * - ROUTE_360_TIME – The Route360°-service is used to calculate the cost of connections as travel times between the individual addresses.
 * Using this source is a lot more accurate than using "AIR_DISTANCE"
 * - default: ROUTE_360_TIME
 * @param {string} geojsonCreation geojsonCreation is an optional setting that can be used to define what Geojson data is created for the
